...
def home(request):
    table_columns = ['date', 'company_name', 'aps_id', 'customer_name', 'property_address', 'type', 'search_number',
                     'created_by_id', 'checked']
    data = pd.DataFrame(columns=table_columns)
    used_models = [AU, CholaMandalam, Poonawalla, Wonder, Aadhar, Axis, Profectus, RBL, HFFC, AYE, Mahindra,
                   YesBankAgri, YesBankSagment, YesBankAHFL, IndiaBulls, NewIndia, AgriBank]
    row_counter = 0
